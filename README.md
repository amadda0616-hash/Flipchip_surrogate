...
a. 3D나 Fan에 의한 유동 해석 등이 포함되면 한 개의 케이스 당 몇 시간씩 걸리게 됩니다.    
이 경우, 딥러닝을 위해 몇천개의 케이스를 해석하려면 가정용 pc 1대로는 몇달 씩 걸립니다.    
따라서 2d로 시뮬레이션을 작업하고 대칭 형상이기 때문에 절반 자른 단면으로 해석을 진행합니다.    
Fan에 의한 냉각이 반영되지 않으면 과열상태가 되어 버려 온도가 급격하게 상승하기 때문에    
윗면에 Fan에 의한 효과를 반영해 줍니다.    

b. flip-chip에 사용된 에폭시 접착제의 경우 고온경화 (높은 온도에서 굳음)이기 때문에 식으면서 발생하는 내부 응력을 반영하기 위해    
처음에 120도로 시작해 상온으로 냉각되고, 100s부터 칩의 핵심인 die 에서 253W (해석의 기준 데이터인 intel i9-13900k 의 최대 출력)을 발생. 최대 100도로 유지되게 출력 제어하고,     
...
d.해석 결과를 바탕으로 주요 위치에 가상 센싱(하드웨어(센서)를 직접 설치하지 않고, 소프트웨어와 알고리즘을 사용해 상태 데이터를 추정해내는 기술)을 하여 데이터를 출력. 아래 22개 컬럼을 생성한다.    
...
# 6. 📈 플립칩(Flip-Chip) 패키지 열변형 3-Step 시나리오 프로파일
...
# 7. 📊 22-Column Simulation Dataset 정의서
