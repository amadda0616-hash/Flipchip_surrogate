...
최종 1,200개의 케이스를 시계열 데이터로 수집합니다. (현재 수집중)      

1) ai 모델 개발
   수집한 데이터를 통해 데이터 분석을 하고 chronos -2, LSTM 등의 딥러닝 모델이나 앙상블 등의 머신러닝 모델들을 몇가지 사용해 데이터를 학습합니다. 이를 통해 뒤틀림과 박리 (y 변수, 16개 컬럼)가 최소화 되는 설계 치수(x 변수, 6개 컬럼)를 찾습니다. 다중 작업 학습 (MTL)구현
2) 모델 별 최적 수치 비교
   모델 별로 구한 최적 수치를 시뮬레이션 모델링에 반영해 기존 1번 해석 결과 (임의의 설계 치수)와 다시 y축 방향 변형(deformation), shear stress (수평방향), normal stress (수직방향)을 비교합니다.    

## 아래는 컴퓨터 시뮬레이션 데이터 도출 과정에 대한 설명과 결과 입니다.
