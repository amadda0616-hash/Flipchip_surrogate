...
## 이외에 이전 nasa turbo fan 수명 예측 데이터셋 추가 해석도 고려 중입니다.
1. 시계열 데이터 분해를 반영
2. chronos - 2 를 사용
3. 웨이불릿 변환을 이용해 특정 수명이 너무 긴 이상치 엔진도 CNN를 이용해 이미지 데이터로 신경망 학습해 예측 이후 역변환으로 수명 예측

위 세가지 방식으로 수명 예측을 해 첫번째 프로젝트 당시의 결과와 성적을 비교
