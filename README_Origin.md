...
역매핑(잠재 변수 $z$를 보고, 그런 결과를 만들어내기 위한 **설계 치수**를 역산하는 것, 즉 최적설계 치수를 구하는 과정) 결과 우수한 선형 형태를 확인할수 있다. P6의 경우 스케일의 차이이고 수치를 보면 소수점 단위로 매우 밀착했음을 알수 있다.
$y=x$ 그래프(대각선 점선)에 수렴한다는 것은 주문한 목표 성능(Target)"과 "AI가 역산출한 설계도로 확인한 실제 성능(Achieved)"이 오차 없이 일치함을 의미합니다. 최적설계 치수를 잘 구했다는 의미입니다.
